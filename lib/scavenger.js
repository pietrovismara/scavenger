...
async function load(options) {
    ...
    try {
        ...
        if (!await util.isUrlReachable(url)) {
...
async function scrape(options) {
    debug('scrape');
    let html, {minify} = parseOptions(options);
    try {
        if (!await webdriver.isLoaded()) {
            await load(options);
        }
        html = await webdriver.getHTML();
    } catch (e) {
        debug(e);
        return Promise.reject(e);
    }
...
async function screenshot(options) {
    debug('screenshot');
    let buffer, {width, crop, format} = parseOptions(options);
    try {
        if (!await webdriver.isLoaded()) {
            await load(options);
        }
        buffer = await webdriver.getScreenshot(width);
        if (crop || format) {
