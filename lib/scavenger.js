...
async function ss() {
    ...
    const results = await Promise.mapSeries(tasks, async (task) => {
        ...
        const opt = _.assign({}, task, options);
        await webdriver.goto(task.url, opt.waitMs);
        await webdriver.waitForElement(opt.selector);
...
async function scrapeOnce(options) {
    const {url, minify, driverFn, nightmareOptions} = parser.parseOptions(options);
    let html, autoEnd;
    try {
...
async function screenshotOnce(options) {
    const {url, width, crop, format, driverFn, nightmareOptions} = parser.parseOptions(options);
    let buffer, autoEnd;
